...
test_that("state-level change calculations keep one 2024 row per state", {
  input <- tibble(
  ...
  expect_identical(sort(output$state), c("CA", "TX"))
  expect_true(all(output$year == 2024))
  expect_true(all(!is.na(output$d_prop_bins)))
})

test_that("total ATR share bins are computed within year", {
  input <- tibble(
    year = rep(2024, 10),
  ...
  expect_equal(round(sum(output$prop_atr_pilots_ofall), 8), 100)
  expect_true(all(!is.na(output$prop_bins)))
})

test_that("mover panel flags moves within pilot-year order", {
  input <- tibble(
  ...
  expect_equal(output$year, c(2009, 2010, 2011, 2014))
  expect_true(is.na(output$origin_state[[1]]))
  expect_identical(output$moved, c(0L, 0L, 0L, 1L))
})

test_that("move counts equal the within-pilot sum of moves", {
  input <- tibble(
  ...
  output <- input |>
    ...
    distinct(.data$unique_id, .data$num_moves)

  expect_equal(output$num_moves[output$unique_id == "p1"], 1)
  expect_equal(output$num_moves[output$unique_id == "p2"], 0)
})

test_that("flow time fields handle non-consecutive years", {
  input <- tibble(
  ...
  expect_true(is.na(output$lag_year[[1]]))
  expect_identical(output$time_period_yrs, c(NA_integer_, 2L, 3L))
})

test_that("mover period summary uses legacy labels and monthly divisors", {
  input <- tibble(
  ...
  expect_identical(output$`Time period`, c("11/2009 - 05/2010", "05/2010 - 09/2011"))
  expect_equal(output$`Ave. monthly % moved`, c((50 / 6), (50 / 16)))
})

test_that("2016-2017 mover period reflects September 2017 timing", {
  input <- tibble(
  ...
  expect_identical(output$`Time period`, "11/2016 - 09/2017")
  expect_equal(output$`Ave. monthly % moved`, 50 / 10)
})

test_that("top-state selection is deterministic on average ATR pilot counts", {
  input <- tibble(
  ...
  expect_identical(output, c("CA", "TX"))
})
